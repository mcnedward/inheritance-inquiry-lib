...
public abstract class Builder {
	...
	protected Future<JavaSolution> submit(Callable<JavaSolution> task) {
		return mExecutorService.submit(task);
	}
		
	protected <T> List<T> invokeAll(Collection<? extends Callable<T>> tasks) throws TaskBuildException {
		List<T> solutions = new ArrayList<>();
		try {
			mExecutorService.invokeAll(tasks, INVOKE_TIMEOUT, INVOKE_TIMEUNIT)
				.stream()
				.map(future -> {
					try {
						return future.get();
	...
	protected void waitForTasks(int jobCount) throws TaskBuildException {
