...
public class ReverseEdgeRenderer<V, E> implements Renderer.Edge<V, E> {
	...
	/**
	 * Draws the edge <code>e</code>, whose endpoints are at <code>(x1,y1)</code> and <code>(x2,y2)</code>, on the
	 * graphics context <code>g</code>. The <code>Shape</code> provided by the <code>EdgeShapeFunction</code> instance
	 * is scaled in the x-direction so that its width is equal to the distance between <code>(x1,y1)</code> and
	 * <code>(x2,y2)</code>.
	 */
